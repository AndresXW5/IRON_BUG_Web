...
if(isset($_POST['submit'])){
    $sql2="UPDATE `historialcompra` SET `precio`='".$rol."',`estado`='".$nombre."' WHERE `id_usuario` like ".$id;
...
<div class="fondo">
    ...
    <?php
    $sql2="SELECT * FROM `historialcompra`";
    $consulta=mysqli_query($con,$sql2);
    while($fila=$consulta->fetch_assoc()){
        echo "Historial de compra del usuario ".$fila["nick"];
        echo "<br></br>";
        echo "<form method ='POST'>
        <label >Estado del pedido</label>
        <input type='text' class='form-styling' name='usuario' placeholder=".$fila["estado"].">
        <br></br>
        <label >Precio</label>
        <input type='text' class='form-styling' name='rol' placeholder=".$fila["precio"].">
        <br></br>
